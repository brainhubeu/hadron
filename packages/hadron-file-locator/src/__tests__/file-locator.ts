...
describe('locate', () => {
  ...
  it('should return an array', () => {
    return configLocate(['./mock/app/config/*'], 'config', 'development').then(result => {
      expect(result).to.be.an('array');
    });
  });

  it('should return path files from mock/app/conifg/* with development type and js extension', () => {
    return configLocate([`${packageDirPath}/mock/app/config/*`],
     'config', 'development', ['JS'])
        .then(result => {
          expect(result).to.deep
            .equal([`${packageDirPath}/mock/app/config/config.js`,
  ...
  it('should return path files from mock/app/config/* with development type and json extension', () => {
    return configLocate([`${packageDirPath}/mock/app/config/*`],
     'config', 'development', ['json'])
        .then(result => {
          expect(result).to.deep
            .equal([`${packageDirPath}/mock/app/config/config.json`,
  ...
  it('should return path files from every folder in mock/plugins with config folder', () => {
    return configLocate([`${packageDirPath}/mock/plugins/*/config/*`],
