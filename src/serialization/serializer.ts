...
export const getConfigurationForType = (type: string, configurations: ISerializationSchema[]) => {
    return configurations.find((configuration: ISerializationSchema) => configuration.name === type);
};
...
export const serializeEntry =
    ...
    if (property.properties && property.type === "array") {
 ...
 */
const serializerProvider = (options: ISerializerConfig = {}) => {
    const parsers = {
        ...defaultParsers,
        ...(options.parsers || {}),
    } as any;
